...
import { addDaysUTC, formatDateOnly, mondayOf } from "@/lib/dates";
...
export default function ShoppingListClient() {
  ...
  return (
    <div className="mt-6 space-y-6">
      ...
      {!loading && menuId && items.length > 0 && (
        <div className="card divide-y divide-black/5 p-2 dark:divide-white/5">
          {pending.map((item) => (
            <label key={item.itemKey} className="flex cursor-pointer items-center gap-3 px-3 py-2.5">
              <input
                type="checkbox"
              ...
              {formatQuantity(item) && <span className="text-sm text-ink-muted">{formatQuantity(item)}</span>}
            </label>
          ))}
          {checked.length > 0 && (
            <div className="pt-1">
              {checked.map((item) => (
                <label key={item.itemKey} className="flex cursor-pointer items-center gap-3 px-3 py-2.5 opacity-50">
                  <input
                    type="checkbox"
                    checked={item.checked}
                    onChange={() => toggleChecked(item)}
                    className="h-4 w-4 rounded border-black/20 text-brand focus:ring-brand/30 dark:border-white/20"
                  />
                  <span className={clsx("flex-1 text-sm", "line-through")}>{item.name}</span>
                  {formatQuantity(item) && <span className="text-sm text-ink-muted">{formatQuantity(item)}</span>}
                </label>
              ))}
            </div>
