...
`./scripts/deploy.sh` builds the Docker image **locally**, saves it as a tarball, and ships it to the host defined by `DEPLOY_HOST`/`DEPLOY_PATH` in `.env` — the remote server never receives source, only the built image plus `compose.yml`, `Caddyfile` and `.env`. This is a real, user-facing action (SSHes to a remote host and starts services) — treat it like any other deploy and don't run it without the user asking. As of this writing the user has explicitly said **not** to deploy yet ("trabajaremos solo en DEV") until they've decided how to handle Caddy — see next paragraph.

**Infra constraint — read before touching anything Docker/Caddy-related:** automenu shares its EC2 host (`nubito`) with the already-live `trip2millionaire`, which already owns host ports 80/443 with its own Caddy (real Let's Encrypt cert). automenu deliberately runs its **own, fully isolated** stack instead of sharing that Caddy: its own Postgres (host port 5433 in dev via `DB_PORT`), its own Caddy on `CADDY_PORT` (default 8443) using `tls internal` (self-signed cert — browsers show a one-time trust warning) since Let's Encrypt can't issue a cert outside 80/443. This was a deliberate tradeoff the user chose (isolation over the RAM/cert benefits of a shared host-level Caddy) specifically to avoid modifying trip2millionaire's working deploy. **Do not** "fix" this by refactoring trip2millionaire's `compose.yml`/Caddy setup or by merging the two Caddy configs unless the user explicitly reopens that decision.

## Architecture

**Auth**: NextAuth with a single `CredentialsProvider` (`src/lib/auth.ts`) checking `username`/bcrypt hash against the `User` table — no OAuth, no admin tier, no multi-user concerns (unlike trip2millionaire, there's no `isAdmin`). JWT session strategy, 30-day maxAge. `src/middleware.ts` gate-keeps everything except `/api/auth`, `/login`, and static assets, redirecting unauthenticated visits to `/login`. Server-side handlers get the current user via `requireUserId()` (`src/lib/session.ts`), which returns `null` (not a throw) when unauthenticated — every API route must check it and return 401 explicitly; there's no shared middleware wrapper for this.

**Data model** (`prisma/schema.prisma`):

- `Dish` — `category` (`PLATO_UNICO`/`PRIMERO`/`SEGUNDO`/`ACOMPANAMIENTO`), `mealType` (`COMIDA`/`CENA`/`AMBAS`), `yieldsTwoMeals`, `active` (the "no disponible ahora" toggle — inactive dishes are excluded from generation but not deleted), plus `DishIngredient[]` (free-text `name` + optional `quantity`/`unit`, no shared ingredient master table — quantities are per-dish, aggregated at shopping-list time by normalized name+unit).
- `WeeklyMenu` — one row per week, `@unique` on `weekStart` (a `@db.Date`, always the Monday of that week).
...
**Route structure**: `src/app/(app)/` is the authenticated shell (`platos`, `menu`, `menu/compra`), wrapped by `src/app/(app)/layout.tsx` (NavBar + children) — no per-route auth checks needed there since `middleware.ts` already covers it. `/login` and the root `page.tsx` (redirects to `/menu`) sit outside that group. API routes under `src/app/api/` are the only place Prisma is queried directly:

- `dishes`, `dishes/[id]` — CRUD; `PATCH` either replaces the whole dish (including a full ingredient replace via delete-then-recreate) or, if the body is exactly `{ active }`, does a fast availability toggle.
