...
A personal weekly menu generator ("automenu") supporting multiple private accounts (e.g. family members), each with their own isolated dish catalog and menus. Next.js 14 App Router + Prisma/Postgres + NextAuth credentials login. Each user maintains a catalog of dishes ("platos") they like to cook, each tagged with a course category and whether it's for lunch/dinner/both; the app randomly assembles a full week's menu from that catalog and derives a shopping list from the chosen dishes' ingredients. UI strings, labels, and dish/category names are all in Spanish — keep new user-facing text consistent with that.
